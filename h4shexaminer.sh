...
compare_hash_files() {
  ...
  for p in "${paths1[@]}"; do
    ...
    if [[ -z "${map2[$p]+x}" ]]; then
      ...
      if [[ "$h1" != "$h2" ]]; then
        any_diff=1
        printf '%s\n' "MISMATCH for file: $p"
        # Display original vs new with differing chars in red
        echo "First :"
        color_diff "$h1" "$h2" | sed -n '1p'
        echo "Second:"
        color_diff "$h1" "$h2" | sed -n '2p'
        echo
      fi
    fi
  done

  # Check files only in second
  for p in "${paths2[@]}"; do
    if [[ -z "${map1[$p]+x}" ]]; then
      printf '%s\t%s\n' "ONLY_IN_SECOND" "$p"
      any_diff=1
  ...
  if (( any_diff == 0 )); then
    ...
    echo -e "${YELLOW}Differences found. See above (mismatches highlighted in red).${RESET}"
  fi
}
